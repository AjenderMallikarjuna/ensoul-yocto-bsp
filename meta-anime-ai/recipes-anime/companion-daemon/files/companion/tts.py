...
# On first call, piper loads the 60 MB ONNX model into the OS page cache.
# Subsequent calls launch a fresh process but the binary + model are already
# cached in RAM, so load time drops from ~37 s to ~9 s on QEMU
# (and to ~0.3 s on real RK3588 A76 hardware).
_PIPER_TIMEOUT = 120  # seconds — enough for slow QEMU emulation
